...
calendarDictionary = {
    'Test' : ConfigParser().default_gcal_id,
    'Test' : 'slkv69pmdu259mf1db7uqr0a8o@group.calendar.google.com', #just typed some random ids but put the one for your calendars here
    'New Test' : 't1f1t6t1iquj8gib735qnqmai4@group.calendar.google.com'
}
...
def makeTaskURL(ending, URL_ROOT):
...
def makeCalEvent(eventName, eventDescription, eventStartTime, sourceURL, eventEndTime, calId):
    ...
    if eventStartTime.hour == 0 and eventStartTime.minute == 0 and eventEndTime == eventStartTime: #only startTime is given from the Notion Dashboard
        if ConfigParser().all_day_event == 1:
            eventStartTime = datetime.combine(eventStartTime, datetime.min.time()) + timedelta(hours=ConfigParser().default_start_time) ##make the events pop up at 8 am instead of 12 am
            eventEndTime = eventStartTime + timedelta(minutes= ConfigParser().default_event_length)
            event = {
                'summary': eventName,
                'description': eventDescription,
                'start': {
                    'dateTime': eventStartTime.strftime("%Y-%m-%dT%H:%M:%S"),
                    'timeZone': ConfigParser().timezone,
                },
                'end': {
...
my_page = notion.databases.query(  #this query will return a dictionary that we will parse for information that we want
    **{
        "database_id": ConfigParser().task_database_id,
        "filter": {
            "and": [
                {
...
if len(resultList) > 0:
    ...
    for i, el in enumerate(resultList):
        ...
        try:
            ...
            ExtraInfo.append("")
        URL_list.append(makeTaskURL(el['id'], ConfigParser().url_root))
        ...
        try:
            ...
            CalendarList.append(calendarDictionary[ConfigParser().default_gcal_name])
        ...
        if CalendarList[i] == calendarDictionary[ConfigParser().default_gcal_name]: #this means that there is no calendar assigned on Notion
                ...
                **{
                    ...
                    "properties": {
                        ...
                        Calendar_Notion_Name:  {
                            'select': {
                                "name": ConfigParser().default_gcal_name
                            },
                        },
                    },
                },
    ...
    log.info("Nothing new added to GCal")
...
for result in resultList:
    ...
    try:
        ...
        calId = ConfigParser().default_gcal_id
    log.info(calId)
...
if len(resultList) > 0:
    ...
    log.info("Nothing new updated to GCal")
...
for result in resultList:
    ...
    try:
        ...
        notion_gCal_CalIds.append(calendarDictionary[ConfigParser().default_gcal_name])
        notion_gCal_CalNames.append(result['properties'][Calendar_Notion_Name]['select']['name'])
...
log.info('test')
log.info(new_notion_start_datetimes)
log.info(new_notion_end_datetimes)
log.info('\n')
for i in range(len(notion_gCal_IDs)):
    log.info(notion_start_datetimes[i], gCal_start_datetimes[i], notion_gCal_IDs[i])
...
log.info(notion_IDs_List)
log.info('\n')
log.info(gCal_CalIds)
...
for i in range(len(calIds)):
    if calIds[i] not in ALL_notion_gCal_Ids:
        ...
        log.info(f'Added this event to Notion: {calName[i]}')
...
if ConfigParser().delete_option == 0 and len(resultList) > 0: #delete gCal event (and Notion task once the Python API is updated)
    CalendarList = []
    CurrentCalList = []
